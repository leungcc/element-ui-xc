...
var _tableColumn = __webpack_require__(240);
  ...
  created: function created() {
    ...
    column.renderCell = function (h, data) {
      ...
      return _self.showOverflowTooltip || _self.showTooltipWhenOverflow ? h(
        ...
        'div',
        { 'class': 'cell' },
        [data.treeNode ? h('span', { 'class': 'el-table__indent', style: { 'padding-left': data.treeNode.indent + 'px' } }) : null, data.treeNode && data.treeNode.isLeaf ? h(
          'div',
          { 'class': ['el-table__expand-icon', data.treeNode.expanded ? 'el-table__expand-icon--expanded' : ''],
            on: {
              'click': function click(e) {
                e.stopPropagation();data.store.toggleTreeExpansion(data.treeNode.rowKey);
              }
            }
          },
          [h('i', { 'class': 'el-icon el-icon-arrow-right' })]
        ) : null, renderCell(h, data)]
      );
    };
  },
